...
export interface LibrefmAuthState {
  /** Whether a Libre.fm API key/secret pair has been saved. `undefined` while still
   * loading, so callers can distinguish "checking" from "unconfigured" — same
   * convention as `useAuth`'s `isConfigured`. */
...
export function useLibrefmAuth(): UseLibrefmAuthResult {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [isConfigured, activeAccount] = await Promise.all([
        window.librefm.isConfigured(),
        window.librefm.getActiveAccount(),
      ]);
      setState((previous) => ({ ...previous, isConfigured, activeAccount, error: undefined }));
    } catch (refreshError) {
